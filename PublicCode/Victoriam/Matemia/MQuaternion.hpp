...
struct VIDECL SQuaternion {
	...
	VIDECL inline  SQuaternion(const ScalarType& pitch, const ScalarType& yaw, const ScalarType& roll) { SVector3 ea(pitch * 0.5F, yaw * 0.5F, roll * 0.5F); auto c = FCos(ea * 0.5F); auto s = FSin(ea * 0.5F); w = c.x * c.y * c.z + s.x * s.y * s.z; x = s.x * c.y * c.z - c.x * s.y * s.z; y = c.x * s.y * c.z + s.x * c.y * s.z; z = c.x * c.y * s.z - s.x * s.y * c.z; }
